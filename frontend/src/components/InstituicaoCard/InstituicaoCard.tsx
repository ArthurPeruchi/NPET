...
interface InstituicaoCardProps {
  ...
  onVerDetalhes: (instituicao: Instituicao) => void;
  onDoar?: (instituicao: Instituicao) => void;
}

export default function InstituicaoCard({ instituicao, onVerDetalhes, onDoar }: InstituicaoCardProps) { 
  return (
    <div className="card-instituicao-container">
      <article className="card-instituicao">
        <div className="card-instituicao-header">
          <div className="card-instituicao-wrapper-logo">
            {instituicao.logo
              ? <img className="card-instituicao-logo" src={instituicao.logo} alt={`Logo da ${instituicao.nome}`} />
              : <div className="card-instituicao-logo-placeholder" aria-hidden="true" />
            }
          </div>
  
          <div className="card-instituicao-body">
            <h3 className="card-instituicao-nome">{instituicao.nome}</h3>
        ...
        <div className="card-instituicao-acoes">
          {onDoar && (
            <Botao
              variant="primario"
              size="md"
